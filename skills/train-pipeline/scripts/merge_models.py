...
@click.option('--verify-encoder/--no-verify-encoder', default=True,
              help='Assert encoder generator_name matches across all models')
    ...
    if verify_encoder and len(models) > 1:
        ...
        for i, m in enumerate(models[1:], 1):
            for key in base_fg:
                ...
                name_base  = getattr(gen_base,  'generator_name', None)
                name_other = getattr(gen_other, 'generator_name', None)
                if name_base != name_other:
