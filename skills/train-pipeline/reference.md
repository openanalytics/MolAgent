...
| Key | Type | Notes |
|---|---|---|
| `Bottleneck` | ONNX encoder | Default. ChEMBL 37, E-logD, v6_best. Best for most endpoints. |
| `Bottleneck_chembl37_base` | ONNX encoder | ChEMBL 37, no logD supervision. **Use for logD, logP, lipophilicity** — the default encoder was trained with logD supervision, which contaminates these endpoints. Also recommended for embedding-driven similarity search. |
| `Bottleneck_chembl27` | ONNX encoder | Legacy ChEMBL 27 encoder. Use when merging with models trained before P1. |
| `rdkit` | Descriptors | ~210 RDKit 2D descriptors. |
| `fps_2048_2` | Fingerprints | Morgan ECFP (2048 bits, radius 2). |
