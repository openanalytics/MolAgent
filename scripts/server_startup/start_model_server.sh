#!/bin/bash
# MolAgent Model Server Startup Script for Unix/Linux
# This script starts the AutoMol model server (port 8001)
...
log "Starting MolAgent Model Server..."
echo "================================="
...
if lsof -i :8001 >/dev/null 2>&1 || netstat -tulpn 2>/dev/null | grep -q :8001; then
    ...
    if command -v lsof >/dev/null 2>&1; then
        ...
        netstat -tulpn 2>/dev/null | grep :8001
    fi
    echo
    echo "Do you want to:"
    ...
    if [ "$choice" = "1" ]; then
        info "Attempting to kill existing process on port 8001..."
        if command -v lsof >/dev/null 2>&1; then
            # Use lsof on systems that have it
            lsof -ti :8001 | xargs -r kill -9
        else
            # Fallback for systems without lsof
            netstat -tulpn 2>/dev/null | grep :8001 | awk '{print $7}' | cut -d'/' -f1 | xargs -r kill -9
        fi
        sleep 2
        
        # Check again if port is free
        if lsof -i :8001 >/dev/null 2>&1 || netstat -tulpn 2>/dev/null | grep -q :8001; then
            error "Could not free port 8001. Please kill the process manually."
            exit 1
        else
            log "Port 8001 is now available."
        fi
    else
...
info "Starting model server on port 8001..."
cd MCP
...
cleanup() {
    log "Shutting down model server..."
...
log "Model server started with PID: $SERVER_PID"
log "Server running at: http://localhost:8001"
