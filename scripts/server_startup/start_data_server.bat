@echo off
REM MolAgent Data Server Startup Script for Windows
REM This script starts the AutoMol data server (port 8000)
...
if not errorlevel 1 (
    ...
    if "!choice!"=="1" (
        echo [INFO] Attempting to kill existing process on port 8000...
        ...
        netstat -ano | findstr :8000 >nul
        if not errorlevel 1 (
            echo [ERROR] Could not free port 8000. Please kill the process manually.
            echo.
            pause
            exit /b 1
        ) else (
...
echo [INFO] Starting data server on port 8000...
cd MCP
...
echo.
echo [INFO] Data server has stopped.
